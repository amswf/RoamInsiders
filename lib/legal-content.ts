...
const privacyEn: LegalDocument = {
  ...
  sections: [
    ...
    { heading: "2. Location information", paragraphs: ["If no departure airport is supplied in the page URL, the comparison page may request an approximate location from ipwho.is using your internet address and use the returned coordinates in your browser to select a nearby scheduled-service airport.", "If you press ‘Use my precise location,’ your browser will ask for permission to share device location. The coordinates are used in your browser to choose a nearby airport and are not stored by TravelGoGuide. You can decline permission and search by city or airport code instead."] },
    { heading: "3. Advertising and URL parameters", paragraphs: ["A visit may include campaign parameters such as UTM values, a Google Click ID (GCLID), a keyword, destination, or airport code. We use these values to prefill the comparison form and preserve campaign attribution. A GCLID or campaign value may be forwarded to Trip.com as an affiliate sub-identifier when you continue your search."] },
    ...
    { heading: "7. Your choices and rights", paragraphs: ["You may decline precise location permission, manually select any airport, remove campaign parameters from the URL, or avoid third-party links. Depending on where you live, you may also have rights to access, correct, delete, restrict, or object to processing of personal information. Contact us to make a request."] },
...
const privacyZhCN: LegalDocument = {
  ...
  sections: [
    ...
    { heading: "2. 位置信息", paragraphs: ["如果页面网址没有指定出发机场，比价页可能通过你的网络地址向 ipwho.is 请求大致位置，并在浏览器中用返回坐标选择附近有定期航班的机场。", "只有你点击“使用我的精确位置”后，浏览器才会请求设备定位权限。坐标仅在浏览器中用于选择附近机场，TravelGoGuide 不会保存。你可以拒绝授权，改为搜索城市或机场代码。"] },
    { heading: "3. 广告与网址参数", paragraphs: ["访问网址可能包含 UTM、Google Click ID（GCLID）、关键词、目的地或机场代码。我们用它们预填表单并保留广告归因；当你继续到 Trip.com 时，GCLID 或活动值可能作为联盟子标识一并传递。"] },
    ...
    { heading: "7. 你的选择与权利", paragraphs: ["你可以拒绝精确定位、手动选择机场、删除网址中的活动参数或不点击第三方链接。你所在地区的法律还可能赋予访问、更正、删除、限制或反对处理个人信息的权利。"] },
